...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small\textit{Indian Institute of Technology Delhi}}
\fancyhead[R]{\small\textit{COP7001 -- Cornerstone Project}}
\fancyfoot[C]{\thepage}
...
\begin{titlepage}
    ...
    {\LARGE \textbf{Indian Institute of Technology Delhi}}\\[0.4em]
    {\large Department of Computer Science and Engineering}\\[2.5cm]

    {\huge \textbf{Accelerated Mobile Video Processing System}}\\[0.6em]
    {\Large Real-Time Camera Pipeline Using CPU SIMD and Mobile GPU}\\[2.5cm]

    {\large \textit{A report submitted for}}\\[0.4em]
    {\large \textbf{Cornerstone Project (COP7001)}}\\[2cm]

    \begin{tabular}{rl}
        \textbf{Submitted by:} & Nitish Kumar \quad (2025MCS2100) \\[0.3em]
                               & Salil Gujar \quad\, (2025MCS2106) \\[1.2em]
        \textbf{Supervisor:}   & Prof.\ Kolin Paul \\[1.2em]
        
    \end{tabular}

    {\small \textbf{Repository:} \href{https://github.com/salil-123/COP7001_Project}{\texttt{https://github.com/salil-123/COP7001\_Project}}}

    \vfill

    {\small Indian Institute of Technology Delhi, Hauz Khas, New Delhi -- 110016}
\end{titlepage}
...
Runtime switching exposed lifecycle issues that would not appear in a single-mode pipeline. We had to fix stale resource reuse, synchronization races, and measurement contamination across different resolutions.
...
\section{Contributions and AI Usage}

\subsection{Team Contributions}

The work was divided equally across all four development phases, with each team member taking ownership of specific stages within each phase.

In Phase 1 (Camera Pipeline and Baseline System), Nitish Kumar set up the Android project and the NDK toolchain, integrated the Camera2 API for continuous frame capture on a background thread, and implemented the YUV-to-RGBA conversion in native C++. Salil Gujar wrote the scalar Sobel edge detection filter including border handling, configured the SurfaceView rendering path, and designed the initial live performance dashboard.

In Phase 2 (SIMD Acceleration), Nitish developed the ARM NEON intrinsics warm-up experiment and wrote the NEON-vectorized Sobel filter that processes sixteen pixel positions at a time using 128-bit vector registers. Salil built the correctness verification framework that compares the SIMD output against the scalar baseline within a tight numerical tolerance, implemented the runtime mode-switching UI, and extended the dashboard with SIMD activity indicators and the speedup ratio display.

In Phase 3 (GPU Acceleration and Hybrid Mode), Nitish set up the headless EGL compute context, wrote the OpenGL ES compute shader for GPU Sobel processing with texture-based input, and implemented the hybrid frame-split execution logic. Salil handled GPU correctness verification, performed sustained load and stress testing across all four mode transitions, and updated the dashboard with GPU and hybrid-specific indicators including the live split-ratio display.

In Phase 4 (End-to-End Optimization and Finalization), Nitish carried out pipeline latency profiling using millisecond timestamps, drove memory and buffer optimization to eliminate garbage-collection pauses, and tuned the NEON filter loops for cache efficiency. Salil worked on GPU shader workgroup-size experimentation and the switch from SSBOs to texture-based access, implemented the dynamic hybrid split-ratio adjustment, and finalized the dashboard layout. The 30-minute final stability run and integration testing were a joint effort by both team members.

\subsection{AI Usage}

We used Large Language Models(Chatgpt, Gemini) as a support tool throughout this project, treating the AI as a collaborative assistant rather than a primary developer.

We designed the core system architecture, the SIMD acceleration strategy, and the GPU integration approach ourselves. In particular, the idea to implement a hybrid execution mode by splitting the camera frame horizontally and routing each half concurrently to the CPU SIMD path and the GPU compute path was entirely our own concept. We also designed the adaptive balancing mechanism that dynamically adjusts the split ratio based on measured per-row throughput of each hardware resource so that both sides finish at roughly the same time. The AI did not suggest or contribute to this hybrid splitting strategy.

For the NEON vectorization path, which we understood well due to our background in CPU-level optimization, the AI was used mainly for debugging, clarifying obscure compiler error messages, and performing static analysis to confirm that our logic was correct.

For the GPU and hybrid modes, since we had less prior experience with the OpenGL ES compute API, the AI helped generate boilerplate code for EGL setup, shader compilation, and buffer management. We provided the high-level design and the hybrid strategy; the AI helped translate those ideas into working GLSL and C++ code. This allowed us to focus on system-level integration and adaptive balancing rather than low-level API mechanics that were new to us.
